    ...
    for quality in sorted((stream.resolution for stream in post.streams.filter(type="video")), key=lambda s: int(s.split("p")[0])):
        ...
        try:
            ...
            if stream is not None:
                ...
                logger.info(f"media with quality: {quality} :: {MediaFormat.MP4} has some problem")
    ...
    for quality in sorted((stream.abr for stream in post.streams.filter(type="audio")), key=lambda s: int(s.split("kbps")[0])):
        try:
            ...
            if stream is not None:
                ...
                logger.info(f"media with quality: {quality} :: {MediaFormat.MP3} has some problem")
            logger.info(f"ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo")
    ...
    while True:
        try:
            ...
            if not post.publish_date:
                ...
                continue
            logger.info(f"++++++++++++++++++++++++++++++++++++++++", extra=log_extra)
            utc_dt_from = as_utc(datetime.fromisoformat((await rds.get(redis_models.source_channel_name_dt_from(Source.YOUTUBE, channel_name))).decode("utf8")))
            utc_dt_to = as_utc(datetime.fromisoformat((await rds.get(redis_models.source_channel_name_dt_to(Source.YOUTUBE, channel_name))).decode("utf8")))
            logger.info(f"{post.publish_date}")
            utc_dt = as_utc(datetime.fromisoformat(str(post.publish_date)))
            if utc_dt_to <= utc_dt:
                logger.info(
            ...
            logger.info(f"get_posts_list({channel_name}) : progress {i:0>4} : {post.video_id}", extra=log_extra)
            yield Post(
...
async def get_channel_posts_list(channel_name: str, *, log_extra: dict[str, str]):
    try:
        utc_dt_from = as_utc(datetime.fromisoformat((await rds.get(redis_models.source_channel_name_dt_from(Source.YOUTUBE, channel_name))).decode("utf8")))
        utc_dt_to = as_utc(datetime.fromisoformat((await rds.get(redis_models.source_channel_name_dt_to(Source.YOUTUBE, channel_name))).decode("utf8")))
        logger.info(f"get_channel_posts_list({channel_name}) -> start {utc_dt_from}...{utc_dt_to}", extra=log_extra)
